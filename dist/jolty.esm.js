...
var baseDestroy = (instance, { remove = false, keepInstance = false } = {}) => {
  ...
  ["autohide", FLOATING, TRANSITION, TELEPORT$1].forEach((key) => {
    if (instance[key]) {
...
class Collapse extends ToggleMixin(Base, COLLAPSE) {
  ...
  _update() {
    ...
    this[TELEPORT$1] = Teleport.createOrUpdate(
      this[TELEPORT$1],
      base,
      opts[TELEPORT$1],
    )?.move(this);
...
class Dropdown extends ToggleMixin(Base, DROPDOWN) {
  ...
  init() {
    ...
    this[TELEPORT$1] = new Teleport(base, { disableAttributes: true });
  ...
  initTab(tab) {
    ...
    const destroy = ({ clean = true, remove = false }) => {
      ...
      if (clean) {
        ...
        tabInstance[TELEPORT$1]?.destroy();
      }
    ...
    const tabInstance = {
      ...
      get initialPlaceNode() {
        return (
          tabInstance[TELEPORT$1]?.placeholder ??
...
export { Collapse, Dialog, Dropdown, Popover, Tablist, Toast, Tooltip };
